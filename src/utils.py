...
def clean_csv(text: Optional[str]) -> str:
    ...
    if not text_str.strip() or text_str.strip() == "None":
        return "Nil"
    # Ersetze mehrere Leerzeichen durch ein Leerzeichen und Semikolons durch Kommas
...
def _positions_can_be_merged(name_a: str, name_b: str) -> bool:
    ...
    if len(short_tokens) == 1 and long_tokens[0] == short_tokens[0]:
        ...
        if extras and all(token in GENERIC_TRAILING_TOKENS or token in LEGAL_ENTITY_TOKENS for token in extras):
            return True
