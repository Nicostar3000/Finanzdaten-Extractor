...
from .portfolio_analysis import is_fee_transaction, is_purchase_transaction
from .utils import clean_csv, choose_canonical_position_name
...
def write_transactions_csv(transactions, output_path):
    ...
    # Positionsnamen koennen je nach Broker leicht anders geschrieben sein.
    # Fuer Auswertungen werden wahrscheinliche Varianten auf einen Namen gemappt.
    positions_aliase = choose_canonical_position_name(
        transaction.get("position") for transaction in transactions
    )

    headers = [
        "Firmenname", "Firmenanschrift", "Telefonnummer", "Fax", "E-Mail-Adresse", "Internetseite",
        "Kundenname", "Kundenanschrift", "Depotnummer",
        "Kaufbeleg-Datum", "Transaktionsdatum", "Uhrzeit", "Handelsplatz",
        "Position (Wertpapier)", "Anzahl", "Kurs", "Endbetrag",
        "Fremdkostenzuschlag", "Verrechnungskonto", "Valuta-Datum", "Gesamtsumme (Soll)",
        "Zusatzinformationen / Fehler", "Quelldatei",
    ]
    ...
    with open(output_file, "w", encoding="utf-8") as file:
        ...
        for transaction in transactions:
            # Jede Zeile wird explizit aus dem internen Transaktionsmodell in das
            # schul-/excelkompatible Zielschema uebersetzt.
