...
from ..common.formatting import choose_canonical_position_name, clean_csv
...
def filter_transactions(
    ...
    if aggregate_filters_active:
        ...
        if top_x is not None:
            ...
            aggregate_positions = (
                aggregate_positions.intersection(top_positions)
                if aggregate_positions
                else set(top_positions)
            )

        allowed_positions = (
            aggregate_positions if allowed_positions is None else allowed_positions.intersection(aggregate_positions)
        )
...
def build_position_chart_data(transactions):
    ...
    for transaction in transactions:
        ...
        entry = position_data.setdefault(
            position,
            {
                "amount": 0.0,
                "anzahl": 0.0,
...
def build_line_chart_data(transactions):
    ...
    for transaction in transactions:
        ...
        broker_data[broker_name][date_obj] = broker_data[broker_name].get(date_obj, 0) + (
